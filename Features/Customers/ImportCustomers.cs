...
public static class ImportCustomers
{
        ...
        : IRequestHandler<Command, Result<ImportResult>>
    {
        public async Task<Result<ImportResult>> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            int districtNameCol = -1, streetAddressCol = -1, landmarkCol = -1, openingBalanceCol = -1;
            ...
            for (int c = 1; c <= totalCols; c++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(request.OpeningBalanceColumn) && header.Equals(request.OpeningBalanceColumn, StringComparison.OrdinalIgnoreCase)) openingBalanceCol = c;
            }
