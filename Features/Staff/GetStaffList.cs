    ...
    internal sealed class Handler : IRequestHandler<Query, Result<object>>
    {
        ...
        public async Task<Result<object>> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            var result = await new QueryBuilder<Entities.StaffMember>(query)
                ...
                .BuildAsync(s =>
                {
                    var staff = (Entities.StaffMember)s;
                    return (object)ToResponse(staff, staff.Branch?.Name ?? string.Empty, staff.ApplicationUser is not null);
                });

            return Result.Success(result);
        }

        private static StaffResponse ToResponse(Entities.StaffMember s, string branchName, bool hasAppAccess) =>
            CreateStaff.Handler.ToResponse(s, branchName, hasAppAccess);
    }
}
